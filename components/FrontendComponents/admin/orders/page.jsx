...
export default function AdminOrdersPage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-4xl font-bold text-gray-800 dark:text-white mb-8">📦 Admin Orders</h1>
      <div className="space-y-6">
        {orders.map((order) => (
          <Card
            key={order.orderID}
            className={`rounded-2xl shadow-md transition duration-200 hover:shadow-lg border-2 ${order.verified ? "border-green-400" : "border-gray-200 dark:border-gray-700"
              }`}
          >
            <CardHeader>
            ...
            <CardContent className="grid md:grid-cols-3 gap-4 mt-4 text-sm">
              ...
              <div className="space-y-1">
                <h4 className="font-semibold text-gray-700 dark:text-gray-200">🧍 Customer</h4>
                <p>{order.name}</p>
                <p>{order.email}</p>
                <p>{order.phone}</p>
              </div>
              {/* Shipping Info */}
              <div className="space-y-1">
                <h4 className="font-semibold text-gray-700 dark:text-gray-200">🚚 Shipping</h4>
                <p>{order.address}</p>
                <p>{order.city}, {order.postal}</p>
              </div>
              {/* Order Info */}
              <div className="space-y-1">
                <h4 className="font-semibold text-gray-700 dark:text-gray-200">🧾 Order Summary</h4>
                <p>Total: <span className="font-medium">${(order.total ?? 0).toFixed(2)}</span></p>
